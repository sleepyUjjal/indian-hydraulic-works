      ...
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer">
            ...
            <span className={`font-heading font-bold text-2xl hidden sm:block transition-colors ${scrolled ? "text-deep-navy" : "text-white"}`}>Indian Hydraulic Works</span>
          </div>
            ...
            <div className="flex gap-6">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className={`text-sm font-semibold transition-colors ${scrolled ? "text-slate-700 hover:text-trust-blue" : "text-slate-300 hover:text-white"}`}
                >
                  {link.name}
                </a>
              ))}
          ...
          <button
            className={`md:hidden focus:outline-none transition-colors ${scrolled ? "text-deep-navy" : "text-white"}`}
